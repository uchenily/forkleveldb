...
namespace {
    ...
    public:
        CountComparator(const Comparator *wrapped)
            : wrapped_(wrapped) {}
...
private:
    ...
    void SnappyUncompress(ThreadState *thread) {
        Uncompress(thread, "snappy", &port::Snappy_Compress, &port::Snappy_Uncompress);
    }

    void ZstdCompress(ThreadState *thread) {
        Compress(thread, "zstd", [](const char *input, size_t length, std::string *output) {
            return port::Zstd_Compress(FLAGS_zstd_compression_level, input, length, output);
        });
    }
    ...
    void Open() {
        ...
        options.compression = FLAGS_compression ? kSnappyCompression : kNoCompression;
    ...
    void ReadWhileWriting(ThreadState *thread) {
        if (thread->tid > 0) {
            ...
            while (true) {
                {
                    std::lock_guard<std::mutex> l(thread->shared->mu);
